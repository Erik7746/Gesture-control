from __future__ import annotations

import logging
import threading
from dataclasses import dataclass, field
from typing import Optional

from mediapipe.tasks.python.vision import PoseLandmarkerResult, HandLandmarkerResult

from .config import Config
from .roi_estimator import ROIEstimator

logger = logging.getLogger(__name__)


@dataclass
class HandState:
    """Estado interno de una mano trackeada."""
    landmarks: list[tuple[float, float, float]]  # 21 landmarks en coords originales
    handedness: str  # "Left" o "Right"
    confidence: float
    center: tuple[float, float]  # centro aproximado en píxeles
    size: float  # tamaño estimado en píxeles (distancia máxima entre landmarks)


class Tracker:
    """Máquina de estados para seguimiento de manos con fallback a detección.

    Thread-safe: los callbacks de MediaPipe LIVE_STREAM pueden actualizar
    resultados desde hilos internos; el bucle principal consulta el estado
    a través de métodos que adquieren un lock.
    """

    def __init__(self, config: Config) -> None:
        self._config = config
        self._roi_estimator = ROIEstimator(config)
        self._lock = threading.Lock()

        # Estado
        self._state = "DETECTING"  # DETECTING | TRACKING | LOST
        self._pose_result: Optional[PoseLandmarkerResult] = None
        self._hand_state: Optional[HandState] = None

        # Contadores
        self._frames_without_hand = 0
        self._frames_without_pose = 0
        self._frames_since_pose_recalibration = 0
        self._frame_counter = 0

        # ROI del frame actual (escrito por el hilo principal, leído por callback)
        self._current_hand_roi: tuple[int, int, int, int] = (0, 0, 0, 0)

    # ── Callbacks llamados desde hilos de MediaPipe ──────────────────────────

    def on_pose_result(self, result: PoseLandmarkerResult) -> None:
        """Callback cuando llega un resultado de PoseLandmarker."""
        with self._lock:
            self._pose_result = result
            if result.pose_landmarks:
                self._frames_without_pose = 0
            else:
                self._frames_without_pose += 1

    def set_current_hand_roi(self, roi: tuple[int, int, int, int]) -> None:
        """Establece el ROI que se está usando en el frame actual para hand detection.

        Debe llamarse desde el hilo principal justo antes de detect_async.
        """
        with self._lock:
            self._current_hand_roi = roi

    def on_hand_result(self, result: HandLandmarkerResult, *args) -> None:
        """Callback cuando llega un resultado de HandLandmarker.

        El ROI utilizado se lee de ``_current_hand_roi`` (escrito previamente
        por el hilo principal) para poder transformar coordenadas.
        """
        with self._lock:
            roi = self._current_hand_roi
            if result.hand_landmarks:
                # Tomar la primera mano detectada (o la de mayor confianza)
                best_idx = 0
                if result.handedness and len(result.handedness) > 1:
                    scores = [cat[0].score for cat in result.handedness]
                    best_idx = int(max(range(len(scores)), key=lambda i: scores[i]))

                lm_norm = result.hand_landmarks[best_idx]
                handedness = (
                    result.handedness[best_idx][0].category_name
                    if result.handedness else "Unknown"
                )
                score = (
                    result.handedness[best_idx][0].score
                    if result.handedness else 0.0
                )

                # Convertir a coords originales
                from .image_utils import transform_normalized_landmarks
                lm_orig = transform_normalized_landmarks(lm_norm, roi)

                # Calcular centro y tamaño aproximado
                xs = [p[0] for p in lm_orig]
                ys = [p[1] for p in lm_orig]
                center = (sum(xs) / len(xs), sum(ys) / len(ys))
                size = max(max(xs) - min(xs), max(ys) - min(ys))

                self._hand_state = HandState(
                    landmarks=lm_orig,
                    handedness=handedness,
                    confidence=score,
                    center=center,
                    size=size,
                )
                self._frames_without_hand = 0
                self._state = "TRACKING"
                logger.debug("Mano detectada: %s (conf=%.2f, size=%.1f)", handedness, score, size)
            else:
                self._frames_without_hand += 1
                logger.debug("HandLandmarker no encontró mano en ROI %s", roi)

    # ── Consultas desde el bucle principal ───────────────────────────────────

    def get_roi_for_hand_detection(self, frame_shape: tuple[int, int]) -> tuple[int, int, int, int]:
        """Devuelve el ROI sobre el cual ejecutar HandLandmarker en el frame actual.

        Llamar una vez por frame desde el hilo principal.
        """
        with self._lock:
            self._frame_counter += 1

            if self._state == "TRACKING" and self._hand_state is not None:
                # ROI basado en última posición conocida de la mano
                roi = self._roi_estimator.from_hand_position(
                    self._hand_state.center,
                    self._hand_state.size,
                    frame_shape,
                )
                # Verificar si debemos perder el tracking
                if self._frames_without_hand >= self._config.hand_lost_threshold:
                    logger.info("Tracking perdido: %d frames sin mano", self._frames_without_hand)
                    self._state = "LOST"
                    self._hand_state = None
                return roi

            # DETECTING o LOST: usar pose si está disponible y fresca
            if self._pose_result is not None and self._frames_without_pose < self._config.pose_lost_threshold:
                rois = self._roi_estimator.from_pose(self._pose_result, frame_shape)
                if rois:
                    # Si hay dos manos, elegir una (por ahora la izquierda si existe, sino derecha)
                    if "left" in rois:
                        return rois["left"]
                    return rois["right"]

            # Fallback: frame completo
            return self._roi_estimator.full_frame(frame_shape)

    def should_run_pose_detection(self) -> bool:
        """Indica si en este frame conviene enviar un frame a PoseLandmarker.

        Estrategia:
        - Siempre en DETECTING/LOST (mientras no tengamos tracking firme).
        - Periódicamente en TRACKING para recalibrar.
        """
        with self._lock:
            if self._state in ("DETECTING", "LOST"):
                return True
            if self._frames_since_pose_recalibration >= self._config.pose_recalibration_interval:
                self._frames_since_pose_recalibration = 0
                return True
            self._frames_since_pose_recalibration += 1
            return False

    @property
    def state(self) -> str:
        with self._lock:
            return self._state

    @property
    def hand_state(self) -> Optional[HandState]:
        with self._lock:
            return self._hand_state

    @property
    def pose_result(self) -> Optional[PoseLandmarkerResult]:
        with self._lock:
            return self._pose_result
